...
public record BalanceService(AccountRepository accountRepository) {
     ...
     * from its current balance. Since the account is contained in a map, we have
     * to update the account using updateAccount method that uses the same id to override
     * the old record in the map.
     *
